...
Activated via `/get-nutshell` in any Claude Code session. Not auto-injected — user must invoke per session (hooks come in Slice 2).

## File Structure

```
.claude-plugin/plugin.json   — Plugin manifest (name, version, author)
skills/get-nutshell/SKILL.md     — All compression rules, ELI5 overlay, examples (SINGLE SOURCE OF TRUTH)
...
- **`skills/get-nutshell/SKILL.md` is the single source of truth.** All compression rules, intensity levels, ELI5 logic, and examples live there. Do not duplicate rules elsewhere.
